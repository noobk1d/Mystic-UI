...
export default function BorderCard({
  ...
  const [eraserAnim, setEraserAnim] = useState(false);
  const [eraserMode, setEraserMode] = useState(false);
  const [drawProgress, setDrawProgress] = useState<number | undefined>(
    undefined
  );
  ...
  }, [strokeWidth, color, onSettingsChange]);

  const handleReset = () => {
    setResetAnim(true);
    signatureRef.current?.clear();
    setTimeout(() => setResetAnim(false), 200);
    setEraserMode(false);
  };
  ...
  const handleEraserClick = () => {
    ...
    setEraserAnim(true);
    setTimeout(() => setEraserAnim(false), 200);
    setResetAnim(false);
  };

  // For HoldToCopyButton: download PNG
  const handleDownloadPng = (progress?: number | "reset") => {
    if (progress === "reset") {
      setDrawProgress(undefined);
      return;
    }
    if (typeof progress === "number") {
      if (progress === 0) {
        setDrawProgress(undefined); // Reset to normal state if hold is interrupted
      } else {
        setDrawProgress(progress);
      }
    } else {
      setDrawProgress(undefined);
      const dataUrl = signatureRef.current?.exportAsPng();
      if (dataUrl) {
      ...
      <CardFooter className="flex justify-between px-3">
        <div className="flex gap-2">
          ...
          <Button
            ...
            className={eraserAnim ? "scale-95 transition-transform" : ""}>
            <svg
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              ...
              <g id="SVGRepo_iconCarrier">
                <path
                  ...
                  fill={eraserMode ? "#222" : "#6c6c6c"}></path>
              </g>
            </svg>
          </Button>
          {/* Reset Button */}
          <Button
            ...
            type="button"
            onClick={handleReset}
            className={resetAnim ? "scale-95 transition-transform" : ""}>
            <svg
              fill="#6c6c6c"
        ...
        <HoldToCopyButton onDownload={handleDownloadPng} />
      </CardFooter>
